...
/**
 * <p>The {@code ArraySlice} class wraps and provides a "view" of an
 * ordinary Java array.  In addition to array-element access, the
 * {@code ArraySlice} class supports the creation of a <i>subarray</i>,
 * which is a contiguous slice of the original array.  The subarray
 * itself is a new {@code ArraySlice} object.  Both the original {@code
 * ArraySlice} and the subarray wrap the same underlying Java array.
 *
 * <p>A {@code ArraySlice} stores a start position (indexed into the
 * underlying array) and a length.  Accesses to the {@code ArraySlice}
...
public class ArraySlice<type T,region R> {
	...
	this.elts = GenericArray.create(length);
	this.start = 0;
	this.length = length;
    }
     ...
     */
    public Elts toArray() pure { return elts; }
