...
public class MemberEnter extends JCTree.Visitor implements Completer {
    ...
    Type signature(List<JCTypeParameter> typarams,
        ...
        Type mtype = new MethodType(argbuf.toList(),
                                    ...
                                    thrownbuf.toList(),
                                    syms.methodClass);
        return (tvars.isEmpty() && rgnvars.isEmpty() && effectvars.isEmpty()) ? 
        	mtype : new ForAll(tvars, rgnvars, effectvars, mtype);
    }
 ...
 *********************************************************************/
    ...
    public void visitVarDef(JCVariableDecl tree) {
        ...
        if ((v.flags_field & FINAL) == 0) {
            if (tree.rpl != null) {
        	attr.attribTree(tree.rpl, env.dup(tree), NIL, Type.noType);
        	tree.sym.rpl = tree.rpl.rpl;
            } else if (tree.sym.owner.kind == TYP) {
        	tree.sym.rpl = rpls.ROOT;
            } else {
        	tree.sym.rpl = RPLElement.StackRPLElement.RPL(syms, tree.sym);
            }
        }
 ...
 *********************************************************************/
        ...
        if (wasFirst) {
            try {
                while (halfcompleted.nonEmpty()) {
                    finishClass(halfcompleted.next());
                }
            } finally {
                isFirst = true;
            }
    ...
    private Env<AttrContext> baseEnv(JCClassDecl tree, Env<AttrContext> env) {
        ...
        if (tree.typarams != null)
            for (List<JCTypeParameter> typarams = tree.typarams;
                ...
                for (DPJRegionParameter param : typarams.head.rplparams) {
                    if (param.sym != null) {
                	typaramScope.enter(param.sym);
                    }
                }
            }
        if (tree.paramInfo != null) { // DPJ
            for (List<DPJRegionParameter> rparams = tree.paramInfo.rplParams;
                 rparams.nonEmpty();
     ...
     */
    private void finishClass(Env<AttrContext> env) {
        JavaFileObject prev = log.useSource(env.toplevel.sourcefile);
        try {
            JCClassDecl tree = (JCClassDecl)env.tree;
