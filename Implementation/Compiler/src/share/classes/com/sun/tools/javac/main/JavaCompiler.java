...
import com.sun.tools.javac.comp.DPJAnnotationCounter;
import com.sun.tools.javac.comp.Attr;
import com.sun.tools.javac.comp.AttrContext;
import com.sun.tools.javac.comp.Check;
import com.sun.tools.javac.comp.CheckEffects;
...
public class JavaCompiler implements ClassReader.SourceCompleter {
    ...
    /** The effect inferencer
     */
    protected CheckEffects checkEffects;
    
    /** The FJTask boilerplate transformation pass.
     */
    protected FJTaskHarness harness;
    
    /** The annotation counter
     */
    protected DPJAnnotationCounter annotationCounter = new DPJAnnotationCounter();
    
    /** The DPJ lowering pass.
     */
    /*protected LowerDPJ lowerDPJ;*/
    ...
    public JavaCompiler(final Context context) {
        ...
        sequential    = options.get("-seq") != null;
        instrument    = options.get("-instrument") != null;
        count         = options.get("-count") != null;
        nondet        = options.get("-nondet") != null;
    ...
    /**
     * Switch:  Should we erase DPJ annotations?
     */
    public boolean eraseDPJ = true;
    
    /**
     * Switch: Should we compile to sequential code?
     */
    public boolean sequential;
    
    /**
     * Switch: Should we instrument sequential code?
     */
    public boolean instrument;
    
    /**
     * Switch: Should we count DPJ annotations?
     */
    public boolean count;

    /**
     * Switch: Should we support nondeterministic DPJ constructs, 
     * and generate code to support them?
     */
    ...
    JavaFileObject printSource(Env<AttrContext> env, JCClassDecl cdef) throws IOException {
        ...
        if (inputFiles.contains(outFile)) {
            ...
            try {
        	int codeGenMode = sequential ? 
        		(instrument ? Pretty.SEQ_INST : Pretty.SEQ) : Pretty.PAR;
        	new Pretty(out, true, codeGenMode).printUnit(env.toplevel, cdef);
                if (verbose)
...
//      System.err.println("completing " + c);//DEBUG
    ...
    public void compile(List<JavaFileObject> sourceFileObject, boolean DPJ)
        throws Throwable {
        compile(sourceFileObject, List.<String>nil(), null, DPJ);
    }
        ...
        throws IOException // TODO: temp, from JavacProcessingEnvironment
    {
        ...
        try {
            ...
            Parser.methodCount = 0;
            delegateCompiler = processAnnotations(enterTrees(stopIfError(
        	    				 ((DPJ && !sequential) ? 
        	    					 makeHarness(parseFiles(sourceFileObjects)) : 
    ...
    private void compile2() {
        try {
            ...
            case ATTR_ONLY:
        	//System.err.println("Policy=ATTR_ONLY");
                checkEffects(attribute(todo));
                break;

            case CHECK_ONLY:
        	//System.err.println("Policy=CHECK_ONLY");
                flow(checkEffects(attribute(todo)));
                break;

            case SIMPLE:
        	//System.err.println("Policy=SIMPLE");
                generate(insertBarriers(desugar(flow(checkEffects(attribute(todo))))));
                break;

            case BY_FILE:
        	//System.err.println("Policy=BY_FILE");
                for (List<Env<AttrContext>> list : groupByFile(flow(checkEffects(attribute(todo)))).values())
                    generate(insertBarriers(desugar(list)));
                break;

            case BY_TODO:
        	//System.err.println("Policy=BY_TODO");
        	while (todo.nonEmpty())
        	    generate(insertBarriers(desugar(flow(checkEffects(attribute(todo.next()))))));
        	break;
            ...
            if (count) {
        	annotationCounter.printStatistics(new PrintWriter(System.err));
            }
        } catch (Abort ex) {
            if (devVerbose)
                ex.printStackTrace();
   ...
   public List<JCCompilationUnit> parseFiles(List<JavaFileObject> fileObjects) throws IOException {
    ...
    /**
     * Generates boilerplate code for DPJ-FJTask compilation
     */
   public List<JCCompilationUnit> makeHarness(List<JCCompilationUnit> roots)
   {
    ...
    public Env<AttrContext> attribute(Env<AttrContext> env) {
        ...
        if (count) {
            annotationCounter.scan(env.toplevel);
        }
    ...
    public List<Env<AttrContext>> checkEffects(List<Env<AttrContext>> envs) {
	ListBuffer<Env<AttrContext>> results = lb();
	for (Env<AttrContext> env : envs)
	    results.append(checkEffects(env));
	return results.toList();
    }
    
    public Env<AttrContext> checkEffects(Env<AttrContext> env) {
        JavaFileObject prev = log.useSource(
                env.enclClass.sym.sourcefile != null ?
                env.enclClass.sym.sourcefile :
                env.toplevel.sourcefile);
        checkEffects.scan(env.tree);
        return env;
    }
     ...
     */
    public boolean suppressErasure = false;
    protected void desugar(Env<AttrContext> env, ListBuffer<Pair<Env<AttrContext>, JCClassDecl>> results) {
        ...
        try {
            ...
            if (!eraseDPJ && !suppressErasure) {
        	// Erase Java 1.5 generics
            ...
            List<JCTree>cdefs = null;
            if (!eraseDPJ) {
        	cdefs = lower.translateTopLevelClass(env, env.tree, localMake);
            } else {
        	cdefs = List.<JCTree>of(env.tree);
            }
    ...
    /**
     * Replaces field read and write operations with calls to special barrier
     * functions, as required for a transactional memory system.
     */
    public List<Pair<Env<AttrContext>, JCClassDecl>> insertBarriers(
	    List<Pair<Env<AttrContext>, JCClassDecl>> envs) {
	if (!eraseDPJ || sequential)
	    return envs;
	
	ListBuffer<Pair<Env<AttrContext>, JCClassDecl>> results = lb();
	for (Pair<Env<AttrContext>, JCClassDecl> env : envs)
	    results.append(insertBarriers(env));
	return results.toList();
    }
    ...
    public void generate(List<Pair<Env<AttrContext>, JCClassDecl>> list, ListBuffer<JavaFileObject> results) {
        boolean usePrintSource = (stubOutput || sourceOutput || printFlat || eraseDPJ);
