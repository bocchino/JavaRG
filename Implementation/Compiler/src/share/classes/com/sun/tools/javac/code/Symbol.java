...
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
    ...
    /** This class represents a symbol that can appear in an RPL element.
     *  
     *  This class also implements RPLElementElement (!) because javac requires all
     *  symbols X to implement an interface XElement that can be used by the
     *  ElementVisitor class.  Here X=RPLElement, so we get the unfortunate 
     *  name RPLElementElement.  The first "Element" comes from DPJ (an RPL element 
     *  is a component of an RPL in the DPJ language) while the second one comes 
     *  from javac (an XElement is the internal representation of program element X).
     */
    public static abstract class RPLElementSymbol extends Symbol
    implements RPLElementElement // Required for conformity with javac
    {
	
	public RPLElementSymbol(int kind, long flags, Name name, Symbol owner) {
	    super(kind, flags, name, Type.noType, owner);
	}
        public <R, P> R accept(ElementVisitor<R, P> v, P p) {
            return v.visitRPLElement(this, p);
        }

    }
    
    /** A class for region name symbols, i.e., the symbol corresponding to the
     *  name r in the declaration 'region r'
     */
    public static class RegionNameSymbol extends RPLElementSymbol {
        ...
        /** Construct a region declaration's symbol, given its flags, name, type and owner.
         */
        public RegionNameSymbol(long flags, Name name, Symbol owner, boolean isAtomic) {
        ...
        /** Clone this symbol with new owner.
         */
        public RegionNameSymbol clone(Symbol newOwner) {
            RegionNameSymbol v = new RegionNameSymbol(flags_field, name, newOwner, isAtomic);
            v.pos = pos;
            return v;
        }
        ...
        public Symbol asMemberOf(Type site, Types types) {
            return new RegionNameSymbol(flags_field, name, owner, isAtomic);
        }

        public ElementKind getKind() {
            long flags = flags();
            if (owner.kind == TYP || owner.kind == ERR) {
                return ElementKind.FIELD_RPL_ELEMENT;
            } else {
                return ElementKind.LOCAL_RPL_ELEMENT;
            }
        }
    }
     ...
     */
    public static class RegionParameterSymbol extends RPLElementSymbol {
    ...
    public static class MethodSymbol extends Symbol implements ExecutableElement {
        ...
        public Symbol asMemberOf(Type site, Types types) {
            Symbol result = new MethodSymbol(flags_field, name, types.memberType(site, this), owner);
            return result;
        }
