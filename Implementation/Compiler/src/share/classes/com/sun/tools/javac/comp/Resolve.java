...
public class Resolve {
    ...
    protected Resolve(Context context) {
        ...
        inferRPL = InferRPL.instance(context);
 ...
 *************************************************************************/
     ...
     *  of underdetermined site In this case, we treat site as an additional
     ...
     *  @param regionargs  The invocation's given region arguments.
	...
	if (useVarargs && (m.flags() & VARARGS) == 0) return null;
        ...
        List<VarSymbol> params = (m instanceof MethodSymbol) ? 
        	((MethodSymbol)m).params() : null;
        ...
        // Capture explicit region args to prevent inconsistency.  For example,
        // <region R>void foo(Data<R> x, Data<R> y) shouldn't be callable
        // like this:  this.<*>foo(a,b), where a:Data<A> and b:Data<B>.
        //
        ...
        if (mt.tag != FORALL && typeargtypes.nonEmpty()) {
            ...
            while (formals.nonEmpty() && actuals.nonEmpty()) {
                ...
                bounds = types.substRPLs(bounds, pmt.rvars, regionargs);
        ...
        if (mt_old.tag == FORALL && (regionargs.nonEmpty() || refGroupArgs.nonEmpty())) {
            ...
            mt = types.substRPLs(pmt.qtype, pmt.rvars, rvars1);            
        }

        // find out whether we need to go the slow route via infer
        boolean instNeeded = tvars.tail != null/*inlined: tvars.nonEmpty()*/;
        boolean RPLInstNeeded = rvars.tail != null;
        ...
        if (instNeeded) {
            regionargs = ((MethodType) mt).regionActuals;
            Type result = infer.instantiateMethod(tvars,
        	    (MethodType) mt,
        	    argtypes,
        	    allowBoxing,
        	    useVarargs,
 ...
 ****************************************************************************/
     ...
     *                 a superclass or implemented interface of site's class.
     */
    Symbol findFieldRegion(Env<AttrContext> env,
                     Type site,
                     Name name,
                     TypeSymbol c) {
        Symbol bestSoFar = regionNotFound;
        Symbol sym;
        Scope.Entry e = c.members().lookup(name);
        while (e.scope != null) {
            if ((e.sym.kind & RPL_ELT) != 0) {
        	return isAccessible(env, site, e.sym)
        		? e.sym : new AccessError(env, site, e.sym);
            }
            e = e.next();
        ...
        Type st = types.supertype(c.type);
        if (st != null && st.tag == CLASS) {
            sym = findFieldRegion(env, site, name, st.tsym);
            if (sym.kind < bestSoFar.kind) bestSoFar = sym;
        }
        for (List<Type> l = types.interfaces(c.type);
             bestSoFar.kind != AMBIGUOUS && l.nonEmpty();
             l = l.tail) {
            sym = findFieldRegion(env, site, name, l.head.tsym);
            if (bestSoFar.kind < AMBIGUOUS && sym.kind < AMBIGUOUS &&
                sym.owner != bestSoFar.owner)
                bestSoFar = new AmbiguityError(bestSoFar, sym);
            else if (sym.kind < bestSoFar.kind)
    ...
    /** Find unqualified region or field with given name.
     * 
     *  // DPJ -- based on findVar
     *  
     *  @param env     The current environment.
     *  @param name    The name of the variable or field.
     */
    Symbol findRegion(Env<AttrContext> env, Name name) {
	Symbol bestSoFar = regionNotFound;
        Symbol sym;
        Env<AttrContext> env1 = env;
        boolean staticOnly = false;
        while (env1.outer != null) {
            if (isStatic(env1)) staticOnly = true;
            Scope.Entry e = env1.info.scope.lookup(name);
            while (e.scope != null && (e.sym.kind & RPL_ELT) == 0)
                e = e.next();
            sym = (e.scope != null)
                ? e.sym
                : findFieldRegion(
                    env1, env1.enclClass.sym.type, name, env1.enclClass.sym);
            if (e.sym instanceof RegionParameterSymbol)
        	sym = e.sym;
            if (sym.exists()) {
                if (staticOnly &&
                    ((sym.kind & RPL_ELT) != 0) &&
        ...
        sym = findFieldRegion(env, syms.predefClass.type, name, syms.predefClass);
        if (sym.exists())
            return sym;
        if (bestSoFar.exists())
            return bestSoFar;
        ...
        for (; e.scope != null; e = e.next()) {
            ...
            if (bestSoFar.kind < AMBIGUOUS && sym.owner != bestSoFar.owner)
                return new AmbiguityError(bestSoFar, sym);
            else if (bestSoFar.kind >= RPL_ELT) {
                origin = e.getOrigin().owner;
                bestSoFar = isAccessible(env, origin.type, sym)
                    ? sym : new AccessError(env, origin.type, sym);
            }
        }
        if ((bestSoFar.kind & RPL_ELT) != 0 && bestSoFar.owner.type != origin.type)
            return bestSoFar.clone(origin);
        else
            return bestSoFar;
    }
    ...
    Symbol mostSpecific(Symbol m1,
        ...
        case MTH:
            ...
            boolean m1SignatureMoreSpecific =
                (instantiate(env, site, m2, types.lowerBoundArgtypes(mt1), null, null,
                             null, allowBoxing, false, noteWarner) != null ||
                 useVarargs && instantiate(env, site, m2, types.lowerBoundArgtypes(mt1), null, 
                	 null, null, allowBoxing, true, noteWarner) != null) &&
                !noteWarner.unchecked;
            Type mt2 = types.memberType(site, m2);
            noteWarner.unchecked = false;
            boolean m2SignatureMoreSpecific =
                (instantiate(env, site, m1, types.lowerBoundArgtypes(mt2), null, null,
                             null, allowBoxing, false, noteWarner) != null ||
                 useVarargs && instantiate(env, site, m1, types.lowerBoundArgtypes(mt2), 
                	 null, null, null, allowBoxing, true, noteWarner) != null) &&
                !noteWarner.unchecked;
     ...
     *                   (a subset of VAR, TYP, PCK, REGION).
     */
    Symbol findIdent(Env<AttrContext> env, Name name, int kind) {
        ...
        if ((kind & RPL_ELT) != 0) {                               // DPJ
            sym = findRegion(env, name);                          // DPJ
            if (sym.exists()) return sym;                         // DPJ
            sym = findVar(env, name);
            if (sym.exists()) return sym;
            //if (sym.exists() && (sym instanceof VarSymbol))
        	//return new VarRegionSymbol(sym.flags_field, (VarSymbol)sym, sym.owner);
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;  // DPJ
        }
    ...
    Symbol findIdentInType(Env<AttrContext> env, Type site,
        ...
        // DPJ BEGIN
        if ((kind & RPL_ELT) != 0) {
            sym = findFieldRegion(env, site, name, site.tsym);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;
        }
        // DPJ END
    ...
    /**
     * Is the given expression in scope?  (DPJ)
     */
    ...
    public boolean isInScope(RegionNameSymbol sym, Env<AttrContext> env) {
	if (!sym.isLocal()) return true;
 ...
 ****************************************************************************/
    ...
    static JCDiagnostic absentKindName(int kind) {
        ...
        case ABSENT_VAR:
            return JCDiagnostic.fragment("kindname.variable");
        case ABSENT_REGION: // DPJ
            return JCDiagnostic.fragment("kindname.region"); // DPJ
